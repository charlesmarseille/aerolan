...
path_aod = 'cosqm_teide/20190901_20210731_Teide.lev10'
...
for i in range(5):
    dtypes = np.array([type(data) for data in cosqm_santa[:,i]])
    cosqm_santa = cosqm_santa[dtypes == float]
    dt_santa = dt_santa[dtypes == float]
...
zeros_mask = ((cosqm_santa>1).all(1)) & ((cosqm_santa<25).all(1))
cosqm_santa = np.array(cosqm_santa[zeros_mask], dtype=float)
...
#    d_mask_pm = np.zeros(ddays_cosqm.shape[0], dtype=bool)
    d_mask_am = (dt_santa_date == day) & (hours_cosqm >= cosqm_min_am_hour) & (hours_cosqm < 4)
    d_mask_pm = (dt_santa_date == day) & (hours_cosqm >= 20) & (hours_cosqm < cosqm_max_pm_hour)
...
plt.scatter(np.unique(dt_santa_date), cosqm_am[:,1], label='cosqm_am', s=15)
plt.scatter(np.unique(dt_santa_date), cosqm_pm[:,1], label='cosqm_pm', s=15)
